...
python3 src/micro_lm/domains/defi/benches/audit_bench.py \
  ...
  --dump_failures --dump_confusion

jq -r '.failure_type' .artifacts/defi/audit_bench_120/failures_thr_*.jsonl | sort | uniq -c | sort -nr

# Inspect a few wrong approvals with low margin
jq -r 'select(.failure_type=="wrong_approval") | {prompt, gold, mapper_top, audit_best, audit_scores_top2, reason}' \
  .artifacts/defi/audit_bench_120/failures_thr_*.jsonl | head -n 20
        ...
        for tau in tau_list:
            for rel in rel_list:
                ...
                if args.dump_failures:
                    fails = _collect_failures(prompts, gold, outs)
                    _write_failures_jsonl(out_dir / f"failures_thr_{thr:.2f}_tau_{tau:.2f}_rel_{rel:.2f}.jsonl", fails, only_problematic=True)
                if args.dump_confusion:
                    _write_confusion_csv(out_dir / f"confusion_approved_thr_{thr:.2f}_tau_{tau:.2f}_rel_{rel:.2f}.csv", gold, outs)
...
def main():
    ...
    ap.add_argument("--dump_failures", action="store_true", help="Write failures_*.jsonl with problematic cases and span details")
