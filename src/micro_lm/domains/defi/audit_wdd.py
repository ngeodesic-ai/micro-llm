...
def wdd_defi_audit(emb: np.ndarray, prototypes: np.ndarray, anchors: np.ndarray, cfg: Dict[str, Any]) -> Dict[str, Any]:
    mode = Mode(cfg.get("mode", "family"))
    T = int(cfg.get("T", 600)); seed = int(cfg.get("seed", 0))
    fams = defi_family_registry(K=prototypes.shape[0]) if mode != Mode.PURE else None
