...
def _null_circ_shift_scores(traces, proto_w, K=48):
...
def build_priors_feature_MFpeak(warp, grids, proto_w=160):
    """
    ...
    for g in grids:
        ...
        k_smooth = min(9, max(3, T//6))
        w_adapt  = min(proto_w, max(13, int(0.7*T)))
...
def wdd_prior_pass_scored(traces, priors,
                          *, z_min=1.7, rel_floor=0.55,  # prior gates
    ...
    if not keep:
        return False, {"keep":[], "order":order, "sigma":sigma, "proto_w":proto_w, "smax":0.0, "z":-999}
...
def decide_arc_with_margins_per_prior(grid, warp, priors, per_prior_thresh, delta_margin=0.10):
    traces = traces_from_grid(grid, warp)
    T = len(traces[0]); _, proto_w = adaptive_windows_short(T)

    scored = []
    for name, pr in priors.items():
        z_gate = per_prior_thresh.get(name, 1.7)
        ok, info = wdd_prior_pass_scored(
            traces, pr,
            z_min=z_gate, rel_floor=0.45, alpha=0.14, beta_s=0.50, q_s=1.8,
            null_K=48
        )
        scored.append((name, info.get("z",-999), info.get("smax",0.0), ok, info))
        ...
        for _, r in rows.iterrows():
            ...
            if family_only:
                ...
                if ok:
                    ...
                    reasons[info.get("reason","fail_family")] += 1
            else:
                ok, info = decide_arc_with_margins_per_prior(
                    g, warp, priors, per_prior_thresh, delta_margin=0.10
                )
                if ok:
                    per_label[lab]["PASS"] += 1
                else:
                    per_label[lab]["ABSTAIN"] += 1
                    reasons[info.get("reason","no_family")] += 1
...
for lab in LABELS:
    ...
    for _, r in rows.iterrows():
        ...
        ok, info = wdd_prior_pass_scored(
            ...
            null_K=48                  # more stable null
        )
        zs.append(info.get("z", -999))
    # pick a robust operating point
    z_min = float(np.quantile([z for z in zs if np.isfinite(z)], 0.88))
